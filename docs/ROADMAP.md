# 路线图

最近更新：2026-07-30

每阶段只推进少量可验证垂直切片，必须通过阶段质量门后再扩大范围。

## 阶段 0：审计（完成）

- [x] 仓库、GitHub、Windows 工具链与执行拓扑审计。
- [x] 前端依赖、质量门、真实浏览器与基线截图。
- [x] localStorage、MarkdownPreview、CodeMirror、字体和持久化审计。
- [x] 产品、UX、设计、编辑器、语言、Markdown、性能、安全和测试文档。
- [x] 固定 Rust 1.95.0 基线全门禁。
- [x] 基线提交、Draft PR 与 CI。

## 阶段 1：专业外壳

1. [完成] 角色化设计 token、月夜深色/暖纸浅色/高对比三套主题，以及
   Comfortable/Compact/Terminal 三档密度；外观独立持久化且切换不重载编辑器。
2. [进行中] Activity Bar + 主侧栏 + 状态栏已落地；大纲/反向链接/局部图谱互斥上下文栏已完成，
   两侧面板宽度可调且跨进程恢复；属性面板与可停靠位置待完成。
3. [完成] 多标签、单槽临时预览、固定/取消固定、按对象上下文菜单和统一 command registry。
4. [进行中] 命令面板、快速打开与可配置的一/二段快捷键编辑器已完成首个 Windows 纵切；
   Vim 模式和移动端命令入口待完成。
5. [进行中] 编辑/阅读/分栏、标签、检查器及两侧面板宽度跨进程布局持久化已完成；
   Zen/Focus/Typewriter 待完成。
6. [进行中] 三主题/三密度、九种视口和 Windows 跨进程外观恢复已完成首轮浏览器/原生验收；
   Android、系统级高对比集成和自动像素差异门待完成。

## 阶段 1.5：声明式交互实验

- [x] `zhiweave-lab` v1 fence、严格 schema、输入上限与失败原文降级。
- [x] UUID v4/v7 结构实验、128 位视图、复制与安全 AI 生成提示词。
- [ ] 算法步进/状态流程组件；每个 kind 单独权限、schema、测试与包体预算。
- [进行中] 已接入统一 Markdown AST 阅读器；导出、同步、版本差异和移动端无障碍待完成。

## 阶段 2：真实本地数据

Workspace 打开 → Markdown 列表 → 原子保存 → expected revision 冲突 → SQLite/FTS → 重建索引 → 文件监控 → 崩溃恢复 → 移除全文 localStorage。

- [x] 固定 Windows 工作区、原子保存、稳定身份、SQLite/FTS 与显式重建。
- [x] watcher 唤醒、完整快照核对和外部更改中心。
- [x] 内容寻址压缩版本 DAG、分支、校验恢复、精确删除和垃圾回收。
- [x] 命名检查点、保留策略精确预览与事务化批量清理。
- [x] 校验清单工作区备份包、恢复前安全备份、重启目录切换与中断继续。
- [ ] 外部备份导入/跨设备恢复演练、备份加密、10,000/100,000 篇性能门。

## 阶段 3–4：编辑器与代码块

- [x] source-preserving 标准 mdast + ZhiWeave Wiki/嵌入/Callout 扩展第一纵切。
- [x] 安全阅读器：GFM、脚注、frontmatter、代码元数据/复制、按需 KaTeX、HTML/资源安全降级。
- [x] Markdown 原文与结构化阅读文本复制；H1/Setext H1 驱动知识节点名。
- [进行中] Corpus 已有首批结构/安全回归和 2 MiB/10,000 行 fence 自动门；官方 CommonMark、
  恶意输入、模糊测试和输入延迟/滚动分布待扩充。
- [进行中] Lezer/mdast 语义适配与 Typora 式 Live Preview：标题/行内/任务/Callout、数学、
  安全静态本地图片/占位、脚注、闭合 fence、composition 停用和全选区揭示已完成；真实 IME/Android
  与点击交互验收待完成。
- [进行中] mdast offset 驱动的大纲 UI、稳定 ID Wiki 反向链接、权威正向打开、missing 目标
  显式创建、受限静态附件预览、受控附件导入/可撤销引用事务和有界一跳局部图谱已完成；
  全局图谱、导出与版本语义 diff 待完成。
- [ ] Language Registry、Tier 1 动态 grammar、缓存、折叠、高亮行和未知语言回退。

## 阶段 5–8：知识与学习

属性/反向链接/关系/集合/Canvas；学习主题与英语切片；FSRS 复习切片；Prompt Composer 隐私预览与候选差异导入。

## 阶段 9–10：同步、安全与发布

威胁模型、设备和密钥、加密对象、版本 DAG、显式冲突、故障注入；最后完成跨平台矩阵、安装包、性能报告、迁移、离线中文 HTML 手册和 RC。
