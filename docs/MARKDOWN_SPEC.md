...
- 局部图谱不是新的 Markdown 语法或关系事实源，而是对当前稳定知识节点及其一跳
  `wiki_edge` 的有界查询视图。删除 SQLite 后重建必须得到等价的节点和边。
- 默认请求最多 40 个节点；Rust 在应用边界拒绝零上限并把任何请求收敛到最多 80 个节点。
  直接邻居按实际引用次数、标题和稳定 ID 确定排序，超过上限时显式返回 `truncated`。
- 相同来源、目标和 link/embed 类型的 occurrence 聚合成一条带次数的有向边；前端不重新解析
  Markdown，也不把已经截断的局部结果冒充全局知识图谱。
- 中心节点与直接邻居都使用稳定 ID。节点可由鼠标、Enter 或 Space 打开；图谱空白右键只显示
  图谱/当前文档操作，具体 SVG 节点右键复用知识节点命令，不继承空白菜单。
- 图谱只在 Tauri + ready index 能力存在时按需加载；浏览器预览不展示无法工作的入口。
  全局图谱仍须另行完成分片、聚类、虚拟化和 10,000/100,000 节点性能门。
...
- Wiki 正向打开、稳定 ID 反向链接、missing 显式新建、受限静态附件解析以及受控附件导入/
  可撤销引用事务和有界一跳局部图谱已接入；全局图谱仍待分片、聚类与虚拟化；
