...
工作台使用独立的 `zhiweave.workbench.preferences.v2` 保存活动稳定节点 ID、编辑/阅读/分栏
模式、实时语法开关、桌面侧栏意图、唯一检查器、版本视图状态，以及 ADR-0017 的标签打开顺序、
关闭历史和预览身份。记录不包含 Markdown、编辑器缓冲、revision、portable path、工作区根、
附件、SQLite 或版本 DAG；原生正文仍只从 Rust 固定工作区加载。旧 v1 四开关记录只读迁移，
不原地改写；未知未来 schema、损坏 JSON 和非法字段回退到安全默认值。
