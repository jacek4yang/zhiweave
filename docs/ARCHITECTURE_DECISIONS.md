...
## ADR-0021：主题与密度是独立、版本化的外观投影

状态：Accepted（阶段 1 设计系统纵切）

颜色主题和界面密度使用独立的 `zhiweave.appearance.v1` 本机记录，只允许
`schemaVersion`、`theme` 和 `density` 三个字段。损坏记录和未来 schema 失败关闭到月夜深色/
紧凑默认值；未知主题或密度分别归一化，不能把正文、路径、稳定节点 ID、revision、工作区根、
附件、索引或版本图带入外观记录。该投影不进入 Markdown、SQLite、版本历史、同步协议或服务端。

主题只在文档根节点切换 `data-theme`/`color-scheme`，密度只切换 `data-density` 与一组有界 CSS
尺寸 token。编辑器、活动标签、选区、滚动位置和 React 知识节点状态不因外观切换而重新创建。
三套主题共享表面、文字、边界、状态和 Markdown 语法角色；高对比模式使用不透明表面、实线边界
和明确焦点，不把透明度或颜色作为唯一状态信号。三档密度分别调整工具栏、标签、列表、状态栏、
编辑器和阅读视图，不改变信息架构或隐藏功能。

外观面板是按需加载的非模态对话框，使用原生 radio 键盘模型、Tab 焦点环、Esc 关闭和触发点
恢复。同一组 command id 同时服务 Activity Bar、状态栏、命令面板和位置相关右键菜单；外观
面板右键只展示精确的主题、密度与恢复命令，状态栏右键只增加打开/恢复入口。
