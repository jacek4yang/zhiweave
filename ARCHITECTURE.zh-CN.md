...
桌面工作台跨进程恢复只保存版本化、有界的 UI 会话：稳定节点 ID、标签/临时预览、编辑模式、
互斥检查器、版本视图与桌面侧栏意图。它不保存 Markdown、编辑器缓冲、revision、路径、附件、
SQLite 或版本 DAG；原生启动必须先取得 Rust 固定工作区快照，再按仍有效的稳定 ID 恢复。
窄视口强制隐藏侧栏属于响应式投影，不能覆盖用户的桌面侧栏意图。
...
- 大批删除列出文件并要求输入确认语句；
- 历史恢复是非破坏性写回，然后创建新提交；
- 本机单笔记版本使用独立可恢复的内容清单，允许精确删除节点并在事务内把直接子节点重接到
  旧父节点；只垃圾回收全局无引用内容块；
- 已同步并签名的跨设备提交不能原地重接或改写；删除这类历史必须通过签名检查点与所有设备
  可见的保留/压缩协议；
